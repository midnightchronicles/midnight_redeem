...
function GetUniquePlayerId(source)
...
function checkadmin(source)
    if source ~= 0 and source ~= -1 and not Bridge.Framework.GetIsFrameworkAdmin(source) then
        return Bridge.Notify.SendNotify(source, Lang["NOTIFY_PERMISSION_DENIED"], "error", 6000)
    end
end
...
AddEventHandler("midnight-redeem:deleteCode", function(code)
    ...
    if src ~= 0 and src ~= -1 and not Bridge.Framework.GetIsFrameworkAdmin(src) then
        return Bridge.Notify.SendNotify(src, Lang["NOTIFY_PERMISSION_DENIED_DELETE"], "error", 6000)
    end
    ...
    exports.oxmysql:execute('DELETE FROM midnight_codes WHERE code = ?', { code }, function(affected)
        if affected and ((type(affected) == "number" and affected > 0) or (type(affected) == "table" and affected.affectedRows and affected.affectedRows > 0)) then
            Bridge.Notify.SendNotify(src, string.format(Lang["NOTIFY_CODE_DELETED"], code), "success", 6000)
            SendToDiscord("Code Deleted", string.format("**Code:** `%s` was deleted by admin <@%s>.", code, GetPlayerName(src)), 15158332)
        else
            Bridge.Notify.SendNotify(src, string.format(Lang["NOTIFY_CODE_NOT_FOUND"], code), "error", 6000)
        end
    end)
end)
...
function GenerateRedeemCode(source, itemsJson, uses, expiryDays, customCode)
    ...
    if not success or type(itemsTable) ~= "table" then
        return Bridge.Notify.SendNotify(source, Lang["NOTIFY_INVALID_ITEM_DATA"], "error", 6000)
    end
    uses = tonumber(uses)
    expiryDays = tonumber(expiryDays)
    if not uses or uses <= 0 then return Bridge.Notify.SendNotify(source, Lang["NOTIFY_INVALID_USES"], "error", 6000) end
    ...
    exports.oxmysql:execute(
        ...
        function(rowsChanged)
            ...
            if affected and affected > 0 then
                TriggerClientEvent("midnight-redeem:notifyUser", source, Lang["NOTIFY_CODE_CREATED"], Lang["NOTIFY_CODE_CREATED"], "success")
                -- Discord log
                local rewardLines = {}
                for _, reward in ipairs(rewards) do
                ...
                Bridge.Notify.SendNotify(source, Lang["NOTIFY_FAILED_INSERT"], "error", 6000)
            end
        end
    )
end
...
RegisterServerEvent("midnight-redeem:redeemCode", function(code, option)
    local src = source
    local uniqueId = GetUniquePlayerId(src)
    ...
    exports.oxmysql:execute('SELECT * FROM midnight_codes WHERE code = ? AND (expiry IS NULL OR expiry > NOW())', { code }, function(result)
        if result[1] then
            ...
            if redeemedBy[uniqueId] then
                return Bridge.Notify.SendNotify(src, Lang["NOTIFY_ALREADY_REDEEMED"], "error", 6000)
            end
            if remainingUses <= 0 then
                return Bridge.Notify.SendNotify(src, Lang["NOTIFY_MAX_USED"], "error", 6000)
            end
            ...
            for _, reward in ipairs(items) do
                ...
                elseif reward.vehicle then
                    ...
                    if model then
                        ...
                        if Config.Framework == "esx" and ESX then
                            playerData = ESX.GetPlayerFromId(src)
                        ...
                        elseif Config.Framework == "qb" and QBCore then
                            playerData = QBCore.Functions.GetPlayer(src)
                            exports.oxmysql:execute('INSERT INTO `player_vehicles` (license, citizenid, vehicle, hash, mods, plate, state, garage) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
                                uniqueId, playerData and playerData.PlayerData.citizenid or "unknown", model, GetHashKey(model), json.encode(props), plate, state, garage
                            })
                        end
            ...
            local notifyMsg = string.format(Lang["NOTIFY_SUCCESSFUL_RECEIVE"], table.concat(receivedSummary, "\n"))
            TriggerClientEvent("midnight-redeem:notifyUser", src, Lang["NOTIFY_CODE_REDEEMED"], notifyMsg, "success")
            ...
            local message = ("**Redeemed By:** `%s`\n**Code:** `%s`\n**Rewards:**\n%s\n**Identifiers:**\n- UniqueID: `%s`"):format(
                safePlayerName, safeCode, safeSummary, safeUniqueId
            )
            SendToDiscord("Code Redeemed", message, 15844367)
        else
            Bridge.Notify.SendNotify(src, Lang["NOTIFY_INVALID_OR_EXPIRED"], "error", 6000)
        end
    end)
end)
...
RegisterServerEvent("midnight-redeem:adminCheckCode", function(code)
    ...
    exports.oxmysql:execute('SELECT * FROM midnight_codes WHERE code = ?', { code }, function(result)
        if result[1] then
            ...
            Bridge.Notify.SendNotify(src, info, Lang["NOTIFY_INFO"], 15000)
        else
            Bridge.Notify.SendNotify(src, Lang["NOTIFY_NO_CODE_FOUND"], "error", 6000)
        end
    end)
end)
